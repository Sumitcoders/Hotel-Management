...
try:
            ...
            if Admin_Login(user,passwd):
                ...
                while True:
                    ...
                    if ch == 1:
                        ...
                        print("3. Remove existing Admin")
                        ch_ad = int(input("Enter Your Choice: "))
                        if ch_ad == 1:
                            cur.execute("select username as 'Admins' from admins")
                            r = cur.fetchall()
                            columns = [i[0] for i in cur.description]
                            print(tabulate(r, headers=columns, tablefmt="fancy_grid"))
                            input("Press 'ENTER' to continue...")
                            ...
                            print(line1)
                            print("!Sensitive Area!")
                            pswd = input("Please Confirm Your Password: ")
                            print(line1)
                            cur.execute(f"Select password from admins where username = '{user}';")
                            if pswd == cur.fetchone()[0]:
                                n_user = input("Enter new username: ")
                                n_password = input("Enter new password: ")
                                n_c_password = input("Confirm password: ")
                                if n_password == n_c_password:
                                    print("New Admin Created Successfully")
                                    cur.execute(f"insert into admins values('{n_user}','{n_password}');")
                                    conn.commit()
                            ...
                            cur.execute(f"Select password from admins where username = '{user}';")
                            if pswd == cur.fetchone()[0]:
                                usr = input("Enter the username to be deleted: ")
                                d_password = input("Enter the password of this username: ")
                                cur.execute(f"select password from admins where username = '{usr}';")
                                if d_password == cur.fetchone()[0]:
                                    if usr == user:
                                        print(line1)
                                        print("Operation failed!")
                                        print("Cannot delete self admin account")
                                        print(line1)
                                    else:
                                        print(line1)
                                        l_c = input(f"Are you sure you want to remove {usr} as admin (y/n): ")
                                        if l_c.lower() == 'y':
                                            captcha = random.randint(100000,999999)
                                            print("Captcha:",captcha)
                                            cap_user = int(input("Enter captcha: "))
                                            if captcha == cap_user:
                                                cur.execute(f"delete from admins where username = '{usr}';")
                                                print(f"Deleted {usr} successfully")
                                                conn.commit()
                                            else:
                                                print("Operation failed!")
                                                print("Wrong Captcha")
    ...
    exit()
print(line3)
print("Thank you for Choosing Hotel Celestial")
print("We hope to see you back in out hotel")
print(line3)
